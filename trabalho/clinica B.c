...
void cadastrar_paciente(ListaDupla* lista_m, NoAVL** raiz_l) {
    ...
    if (sexo == 'M') {
        inserir_ordenado(lista_m, paciente);
        printf("Paciente cadastrado na lista do Moisés.\n");
    } else if (sexo == 'F') {
        *raiz_l = inserir_avl(*raiz_l, paciente);
        printf("Paciente cadastrado na árvore da Liz.\n");
    } else {
...
void alterar_registro(ListaDupla* lista_m, NoAVL* raiz_l) {
    ...
    if (paciente != NULL) {
        printf("Paciente encontrado na lista do Moisés.\n");
    } else {
        // Busca na árvore da Liz
        paciente = buscar_avl(raiz_l, nome);
        if (paciente != NULL) {
            printf("Paciente encontrado na árvore da Liz.\n");
        } else {
            printf("Paciente não encontrado.\n");
            return;
        }
    ...
    printf("Digite o novo sexo (M/F) (ou pressione Enter para manter o atual): ");
    char novo_sexo;
    scanf(" %c", &novo_sexo);
    ...
    printf("Digite a nova data de nascimento (dd/mm/aaaa) (ou pressione Enter para manter a atual): ");
    char nova_nascimento[11];
    scanf(" %s", nova_nascimento);
    if (strlen(nova_nascimento) > 0) {
        strcpy(paciente->nascimento, nova_nascimento);
    }

    printf("Digite a nova data da última consulta (dd/mm/aaaa) (ou pressione Enter para manter a atual): ");
    char nova_consulta[11];
    scanf(" %s", nova_consulta);
    if (strlen(nova_consulta) > 0) {
